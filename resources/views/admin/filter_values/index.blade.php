            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        {{-- Only show Add button if user has access --}}
                        @if($filterValuesModule['full_access'] ?? false || $filterValuesModule['edit_access'] ?? false)
                        <a style="max-width: 200px; float:right; display:inline-block;" 
                        href="{{route('filter-values.create', $filter->id)}}" class="btn btn-primary">
                        Add Filter Value
                      </a>
                      @endif
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{-- Edit --}}
                                        @if($filterValuesModule['edit_access'] ?? false || $filterValuesModule['full_access'] ?? false)
                                        <a href="{{ route('filter-values.edit', [$filter->id, $value->
                                        id]) }}" class="btn btn-sm btn-warning">
                                         <i class="fas fa-edit"></i>
                                        </a>
                                        @endif
                                        {{-- Delete --}}
                                        @if($filterValuesModule['full_access'] ?? false)
                                        <form action="{{ route('filter-values.destroy', [$filter->id, 
                                        $value->id]) }}" method="POST" style="display:inline-block">
                                            @csrf
                                            @method('DELETE')
