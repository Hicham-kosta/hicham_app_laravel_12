              ...
              <div class="card-header">
                <h3 class="card-title">Banners</h3>
                 @if($bannersModule['edit_access'] == 1 || $bannersModule['full_access'] == 1)
                  <a href="{{route('banners.create')}}" class="btn btn-primary float-end">
                    Add Banner
                  </a>
                 @endif    
              </div>
                  ...
                  <tbody>
                    @foreach($banners as $banner)
                      <tr>
                        ...
                        <td>
                          @if($bannersModule['edit_access'] == 1 || $bannersModule['full_access'] == 1)
                            <a class="updateBannerStatus" data-banner-id="{{ $banner->id }}"
                             href="javascript:void(0)">
                             <i class="fas fa-toggle-{{ $banner->status ? 'on' : 'off' }}" 
                             style="color: {{ $banner->status ? '#3f6ed3' : 'grey' }}" 
                             data-status="{{ $banner->status ? 'Active' : 'Inactive' }}"></i>
                            </a>
                          @endif
                        </td>
                        <td>
