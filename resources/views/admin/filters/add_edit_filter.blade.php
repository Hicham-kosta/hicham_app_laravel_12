                ...
                <div class="col-sm-6"><h3 class="m-0">Catalogue Management</h3></div>
                 <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-end">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{</li>{{$title}}</li>
                    </ol>
                 </div> 
            </div>
        </div>
                ...
                <div class="col-md-8">
                    <div class="card card-primary card-outline mb-4">
                        <div class="card-header">
                         ...
                         @endforeach
                         <form action="{{isset($filter) ? route('filters.update', $filter->id) : 
                            route('filters.store')}}" method="post">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category_ids[]" class="form-control" multiple>
                                    @foreach($categories as $cat)
                                        <option value="{{$cat->id}}" 
                                        @if(in_array($cat->id, old('category_ids', $selectedCategories ?? [])))
                                        selected @endif>
                                        {{$cat->name}}
                                        </option>
                                        @if(!empty($cat->subcategories))
                                            @foreach($cat->subcategories as $subcat)
                                                ...
                                                @if(in_array($subcat->id, old('category_ids', $selectedCategories ?? [])))
                                                selected @endif>
                                                &nbsp;&nbsp;--&nbsp;{{$subcat->name}}
                                                </option>
                                            @endforeach
