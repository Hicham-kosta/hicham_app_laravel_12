...
class Category extends Model
{
    ...
    public static function categoryDetails($url){
        $category = self::with(['subcategories' => function($q){
            $q->with(['subcategories:id,parent_id,name']); // Nested subcategories
        }])
        ->where('url', $url)
        ->where('status', 1)
        ->first();
        ...
        $catIds = [$category->id];
        // Add first level subcategories IDs
        foreach($category->subcategories as $subcat){
            $catIds[] = $subcat->id;
            // Add second level subcategories IDs
            foreach($subcat->subcategories as $subsubcat){
                $catIds[] = $subsubcat->id;
            }
        }
        //echo "<pre>"; print_r($catIds); die; // Debugging line, can be removed later
           
        $breadcrumbs = '<div class="px-2 py-1 mb-1" style="background-color: #f9f9f9;">';
        $breadcrumbs .= '<nav aria-label="breadcrumb">';
        $breadcrumbs .= '<ol class="breadcrumb mb-0" style ="background-color: #f9f9f9;
        --bs-breadcrumb-divider: \'>\';">';
        $breadcrumbs .= '<li class="breadcrumb-item"><a href="' . url('/') . '" class="text-dark 
        text-decoration-none">Home</a></li>';

        if ($category->parent_id == 0){
            $breadcrumbs = '<li class="breadcrumb-item active" aria-current="page"><strong>' . $category->name . 
            '</strong></li>';
        }else{
